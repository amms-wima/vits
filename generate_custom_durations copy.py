...
# # Add this temporarily before the inference loop to inspect the model
# print("Available attributes containing 'emb' or 'g':")
# for name, module in net_g.named_modules():
#     if 'emb' in name or 'speaker' in name:
#         print(f"  {name}: {type(module)}")

# # Specifically check the encoder
# if hasattr(net_g, 'enc_p'):
#     print("\nEncoder attributes:")
#     for name, module in net_g.enc_p.named_modules():
#         if 'emb' in name:
#             print(f"  enc_p.{name}: {type(module)}")   


# # Add this to your script to find the correct layer
# print("Searching for 256-channel embedding...")
# for name, module in net_g.named_modules():
#     if isinstance(module, torch.nn.Embedding):
#         embed_dim = module.weight.shape[1]
#         num_embeddings = module.weight.shape[0]
#         print(f"Found Embedding: {name} -> Shape [{num_embeddings}, {embed_dim}]")
#         if embed_dim == 256 and num_embeddings >= 130:
#             print(f"  >>> THIS IS THE SPEAKER EMBEDDING: {name}")   


print("Searching for speaker embedding weights [130, 256]...")
for name, param in net_g.named_parameters():
    if param.shape == (130, 256) or param.shape == (256, 130):
...
with open(INPUT_CSV, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        with torch.no_grad():
            ...
            # B. Get Durations from Model (Stochastic or Standard)
            if hps['model'].get('use_sdp', True):
                # Stochastic Duration Predictor (Inference mode: reverse=True)
                # noise_scale=0.667 is standard, lower for less variation
                logw_ = net_g.dp(x, x_mask, g=None, reverse=True, noise_scale=0.667)
            else:
                # Standard Duration Predictor
                logw_ = net_g.dp(x, x_mask, g=None)
            
            # Convert log-duration to linear duration
            w = torch.exp(logw_) * x_mask
            w_ceil = torch.round(w)
            
            # --- CRITICAL: OVERWRITE PUNCTUATION DURATIONS ---
            # w_ceil shape: [Batch, 1, Time_Text]
            # durations shape: [Batch, Time_Text] (from your previous step)
            for i in range(durations.shape[1]):
                if durations[0, i] > 0:
                    w_ceil[0, 0, i] = durations[0, i]
            
            # Calculate audio lengths and mask
            y_lengths = w_ceil.sum([1, 2]).long()
            max_len = int(y_lengths.max())
            y_mask = mask_from_lengths(y_lengths, max_len).unsqueeze(1).to(x_mask.dtype)
            
            # C. Expand Latent Variable
            # z_p = m_p + torch.randn_like(m_p) * torch.exp(logs_p) * 0.667
            # z_p = m_p 
            z_p = m_p + torch.randn_like(m_p) * torch.exp(logs_p) * 0.01   
            
            # Get repeats as a 1D tensor: [Time_Text]
            # w_ceil is [1, 1, Time_Text], so w_ceil[0, 0, :] gives [Time_Text]
            repeats = w_ceil[0, 0, :].long()
            
            # Expand along the time dimension (dim=2)
            z = torch.repeat_interleave(z_p, repeats, dim=2)
            
            # Update y_lengths and y_mask to match the new expanded length
            y_lengths = torch.sum(repeats).unsqueeze(0) # Shape: [1]
            max_len = int(y_lengths[0])
            y_mask = mask_from_lengths(y_lengths, max_len).unsqueeze(1).to(x_mask.dtype)
            
            # Trim or pad if necessary (usually exact match with repeat_interleave)
            current_len = z.shape[2]
            if current_len != max_len:
                if current_len < max_len:
                    z = torch.nn.functional.pad(z, (0, max_len - current_len))
                else:
                    z = z[:, :, :max_len]   

            # D. Decoder (Vocoder)
            # net_g.dec expects (z, y_mask, [g])
            # audio = net_g.dec(z, y_mask, g=None)
            # audio = net_g.dec(z, None, y_mask) 
            # audio = net_g.dec(z, None)
            ...
            write(out_path, sampling_rate, audio_int16)
            print(f"Generated: {filename} (Text Len: {len(text_ids)}, Audio Len: {len(audio_int16)/sampling_rate:.2f}s)")   
