...
class SuttaSegmentor:
    """
    Segmentor tailored for English Sutta translations.
    Handles:
    - Sentence terminals (. ! ? … ;)
    - Non-citation colons (: but not 1:21 or 2:1)
    - Em-dashes (—) and En-dashes (–)
    - Inline & line-start bullets (•)
    - Translator glosses & parentheticals ({...}, [...], (...))
    - Compound trailing punctuation clusters (...?', ].)
    """

    def __init__(self):
        # Comprehensive boundary regex:
        # 1. Paragraph breaks (\n\s*\n)
        # 2. Before bullets (?=\s*•)
        # 3. After em-dashes / en-dashes (?<=[\—\–])
        # 4. Before opening brackets/parentheses (?=\s*[\(\[\{])
        # 5. After closing brackets/parentheses (?<=[\)\}\]][\.\!\?\;\:\,\…\'"’\”]*)(?=\s+|$)
        # 6. Non-citation colons (?<!\d)\:(?!\d)\s*
        # 7. Sentence terminals (. ! ? ; …) + trailing quotes/brackets
        ...
        self.has_alphanumeric = re.compile(r'[\w\d]', re.UNICODE)

    def split_into_segments(self, text: str) -> List[str]:
        if not text or not text.strip():
            return []

        # Split on boundary points
        raw_chunks = self.boundary_pattern.split(text)
        ...
        # Failsafe pass: Merge orphan punctuation/quote chunks into the previous segment
        clean_segments: List[str] = []
        for seg in raw_segments:
            if not self.has_alphanumeric.search(seg):
                if clean_segments:
                    clean_segments[-1] += " " + seg
                else:
...
class TextToSpeech:
# Baseline pause values for trailing punctuation symbols (in seconds)
    PUNCTUATION_PAUSES = {
        '…': 1.00,  # Sutta elision / refrain
        '.': 0.80,  # Sentence terminal
        '!': 0.80,
        '?': 0.80,
        ';': 0.50,  # Major clause
        ':': 0.50,
    ...
    _sutta_segmentor = None

    def __init__(self, config: any):
        self._config = config
        self._sutta_segmentor = SuttaSegmentor()
    ...
    @classmethod
    def calcPunctuatePauseLengths(cls, segment: str, is_header: bool = False) -> Tuple[float, float]:
        """Returns (pre_seg_pause, post_seg_pause) in seconds.
        Collapses compound trailing marks to their maximum duration.
        """
        if is_header or (segment.isupper() and len(segment) > 3):
            return 0.50, 2.00

        pre_pause = 0.05
        post_pause = 0.20  # Default short clause boundary

        # --- PRE-PAUSE LOGIC ---
        if segment.startswith('•'):
            pre_pause = 0.50
        elif segment.startswith(('(', '[', '{')):
            pre_pause = 0.15  # Micro-pause before translator gloss

        # --- POST-PAUSE LOGIC (MAX COLLAPSE) ---
        tail = segment[-10:] if len(segment) >= 10 else segment
        matched_pauses = [
            pause for char, pause in cls.PUNCTUATION_PAUSES.items() 
            if char in tail
        ]

        if matched_pauses:
            post_pause = max(matched_pauses)
    ...
    def synthesize(self, text, concat_audio=None):
        ...
        text_segments = self._sutta_segmentor.split_into_segments(text)
        ipa_text = ""
        ...
        for i, text_seg in enumerate(text_segments):
            ...
            pre_seg_pause, post_seg_pause = TextToSpeech.calcPunctuatePauseLengths(trimmed_seg)
            concat_audio = self._concat_audio_segment(
                concat_audio, pre_seg_pause, seg_audio, post_seg_pause
            )
            if self._config.verbose:
                logger.debug(f"\nsegment: {trimmed_seg}")
                logger.debug(f"\tipa: {ipa_seg}")
                logger.debug(f"\tprep-pause: {pre_seg_pause}")
                logger.debug(f"\tpost-pause: {post_seg_pause}")
    ...
    def _concat_audio_segment(self, concat_audio: np.ndarray | None, 
                              pre_seg_pause: float, 
                              seg_audio: np.ndarray, 
                              post_seg_pause: float) -> np.ndarray:
        if getattr(self._config, 'no_infer', False):
            return concat_audio

        sr = self._hps.data.sampling_rate
        
        # Calculate sample lengths
        pre_samples = int(pre_seg_pause * sr)
        post_samples = int(post_seg_pause * sr)
        
        pre_pause_audio = np.zeros(pre_samples, dtype=seg_audio.dtype)
        post_pause_audio = np.zeros(post_samples, dtype=seg_audio.dtype)

        # Assemble segment with pre/post silences
        framed_segment = np.concatenate((pre_pause_audio, seg_audio, post_pause_audio))

        if concat_audio is None:
            return framed_segment
        else:
            return np.concatenate((concat_audio, framed_segment))
