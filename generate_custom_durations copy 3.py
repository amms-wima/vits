...
# ═══════════════════════════════════════════════════════════
# WRAPPED DURATION PREDICTOR
# ═══════════════════════════════════════════════════════════
class WrappedDP(torch.nn.Module):
    def __init__(self, original_dp, log_durations_override):
        super().__init__()
        self.original_dp = original_dp
        self.log_durations_override = log_durations_override
        self.override_mask = (log_durations_override > 0).float()

    def forward(self, x, x_mask, w=None, g=None, reverse=False, noise_scale=1.0):
        logw = self.original_dp(x, x_mask, w=w, g=g, reverse=reverse, noise_scale=noise_scale)
        logw = torch.where(
            self.override_mask.bool(),
            self.log_durations_override,
            logw
        )
        return logw
